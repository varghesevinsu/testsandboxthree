...
export class DepartmentsDetailBaseComponent {
  ...
  pageViewTitle: string = 'DEPARTMENTS_DETAIL';
  ...
  workflowActionBarAction(btn: any) {
    ...
    let action: Exclude<keyof DepartmentsDetailBaseComponent, ' '> = methodName;
  ...
  editFormula(config: any, fieldname: any) {
    ...
    if (fieldConfig.formulaMethod) {
      let methodName: Exclude<keyof DepartmentsDetailBaseComponent, ' '> = fieldConfig.formulaMethod
  ...
  onSave(isToastNotNeeded?: boolean) {
    ...
    if (this.checkValidations()) {
      ...
      if (Object.keys(splittedData.files).length > 0) {
        ...
        const saveSubscription = this.departmentsService[method](requestedObj).subscribe((res: DepartmentsBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
  ...
  uploadAttachmentsandSaveData(data: any, splittedData: any): Observable<any> {
    ...
    if (!this.id) {
      const saveSubscription = this.departmentsService.create(splittedData.data).subscribe(
        createdData => {
  ...
  updateData(splittedData: any) {
    ...
    const updateSubscription = this.saveFiles(splittedData).subscribe((dataToUpdate: any) => {
      ...
      this.departmentsService.update(dataToUpdate.data).subscribe(
        res => {
          subject$.next(res);
          subject$.complete();
        },
  ...
  getData() {
    if (environment.prototype && this.id) {
      ...
      this.departmentsService.getProtoTypingDataById(params).subscribe((res: any) => {
      ...
      const dataSubscription = this.departmentsService.getById(params).subscribe((res: DepartmentsBase[]) => {
        this.data = res || {};
        this.formatRawData();
      });
